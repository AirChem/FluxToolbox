...
% 20190401 GMW  Fixed bin averaging bug in SE and RE averages.
...
for i = 1:Ndiv
    ...
    [RE_avg,~,Navg] = BinAvg(leg(iavg),RE(iavg).^2,allLegs);
    RE_avg = sqrt(RE_avg./Navg);
    RE_avg = RE_avg(fitLegs);
    SE_avg = BinAvg(leg(iavg),SE(iavg),allLegs); 
    SE_avg = abs(SE_avg(fitLegs).*flux_avg); %now in flux units
    ...
    if any(isnan(flux_avg))
        warning('FluxDivergence:MissingData',...
            ['Cannot perform divergence fit for legs [%s]. ' ...
            'At least one leg does not contain valid fluxes in the area of overlap. ' ...
            'Check usedpts output.'],num2str(fitLegs))
    end
end
